...
- **Four resource packs** — `Ore Pack`, `Bar Pack`, `Herb Pack`, `Rune Pack`.
  Each hands out 3 level-biased rolls from one resource table (Smithing for ores
  and bars, Herblore for herbs, Magic for runes). Ores and bars are new
  categories; herbs and runes absorb the old `herb_supplies`/`runes`. Coal
  carries the biggest quantities in the table on purpose — the gathering,
  processing and drop shuffles all make raw materials harder to get, never
  easier.
- **The level gate is now a bias, not a rule.** The reward roll no longer filters
  ...
  stops coming.
- **Filler is five named Archipelago items** instead of one opaque
...
- **Quantities vary.** New `qty_max` column; every reward rolls a uniform amount
  in `[qty, qty_max]`. The table grew from 175 rows / 14 categories to 201 / 16
