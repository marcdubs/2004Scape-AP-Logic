...
// JSON edge list for entrance randomization. Not a general rs2 parser - it only
// understands the handful of constructs actually used in that folder (switch_coord,
// switch_int, if/else coord checks, p_telejump/p_teleport/~climb_ladder calls, and the
// @stair_options/@ladder_options two-way choice labels). See ARCHIPELAGO_IDEAS.md #1.

const CONTENT_ROOT = path.resolve(process.cwd(), '../content');
const ENTRANCE_DIR = path.join(CONTENT_ROOT, 'scripts/ladders+stairs/scripts');
...
type CoordExpr = CoordLiteral | CoordRelative | CoordUnknown;

type SourceSpec = { type: 'literal'; coord: CoordLiteral } | { type: 'angle'; angle: number } | { type: 'any' };
...
function decodeCoord(raw: string): CoordLiteral {
...
function parseCoordExpr(raw: string): CoordExpr {
...
function parseFile(filePath: string): Entrance[] {
    const text = readSource(filePath);
    const relFile = path.relative(CONTENT_ROOT, filePath);
...
function main() {
    ...
    const entrances = files.flatMap(parseFile);
