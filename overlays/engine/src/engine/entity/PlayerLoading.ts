...
import { applyNewPlayerSetup } from '#/engine/ApNewPlayer.js';
...
export class PlayerLoading {
    ...
    static load(name: string, sav: Packet, client: ClientSocket | null) {
        ...
        if (sav.data.length < 2) {
            ...
            // AP: skip Tutorial Island for brand-new accounts (no-op unless enabled).
            if (Environment.node.apSkipTutorial) {
                applyNewPlayerSetup(player);
            }
            return player;
        }
        ...
        // AP: skip Tutorial Island for accounts that logged out mid-tutorial (no-op
        // unless enabled; also a no-op here for anyone already past the tutorial).
        if (Environment.node.apSkipTutorial) {
            applyNewPlayerSetup(player);
        }
