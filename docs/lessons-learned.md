...
## Session addendum (2026-07-18b): full ../Server coverage audit

User asked "is anything else not in this repo?" - audited every dirty/untracked
file across all five git repos in ../Server (root, engine, content, webclient,
javaclient) against overlays/. Results:

- **Two real gaps, both fixed**: `engine/tools/map/GenerateDungeon.ts` (+
  `dungeon-example.json`) existed only in ../Server (referenced by the
  ClientCheatHandler test-dungeon cheat) - now overlay files. And
  `content/pack/obj.pack` with the hand-appended AP ids 3894-3901 existed only
  as an uncommitted ../Server edit - now shipped as
  `overlays/content/pack/obj.pack`. **New rule: hand-append new obj ids in the
  OVERLAY obj.pack, then install** - never only in ../Server.
- **varp.pack deliberately NOT shipped**: it regenerates byte-identically from
  `rm` + rebuild (deterministic crawl, no cert-style side entries), and a
  shipped copy would go stale and revert-clobber on every install whenever a
  varp is added.
- Everything else is fine: root/webclient/javaclient "modified" files are pure
  CRLF noise (`git diff --ignore-cr-at-eol` is empty); engine's real diffs are
  exactly the 7 overlay-covered files (byte-identical to overlays); content's
  ~224 modified .npc/.rs2 files are randomizer OUTPUT (spawn/drop/gather/
  process rewrites, originals in content/.ap-backup/), `ap_mimic.rs2` is
  generated by RandomizeDrops, and the engine seed jsons
  (entrance/drop/gather/shop/drip/process-seed.json), tools/map/entrances.json
  and public/ap/worldmap-* are all generated by overlay tools. Runtime state
  (engine data/config/ap-*.json, data/players) is gitignored per-seed state,
  deliberately not repo material.
...
## Addendum (2026-07-20): fresh-install audit - varp.pack was missing

A brand-new Windows install following the README died at the first pack build:
"Missing varp pack IDs" for every ap_* varp. Root cause: the AP varp
registrations lived only in the working ../Server checkout's
content/pack/varp.pack; obj.pack had been captured into overlays/ long ago but
varp.pack never was. Fixed by shipping the full registry as
overlays/content/pack/varp.pack (382 entries - the packer renumbered the whole
file when the AP varps were added, so it ships as a unit, byte-identical to
the validated local file; .gitattributes `* -text` keeps it that way).

How to audit for this class of bug (run it after adding any new pack ID /
engine file / content file):

- `git status --short` in each ../Server sub-checkout (engine, content,
  webclient, javaclient) lists every local modification.
- Each modified file must be either (a) byte-identical to its overlays/ copy
  (`cmp`), or (b) a known generated artifact. The generated set: content
  .npc configs and `scripts/drop tables/scripts/*.rs2` (RegenerateAll drip/
  shop/drop mutations + seed-independent mimic preambles - regenerated by
  new-run.sh RUN_CONTENT, NOT overlay material) and engine data/ runtime
  state. Anything else unaccounted for = a missing overlay = broken fresh
  install.
